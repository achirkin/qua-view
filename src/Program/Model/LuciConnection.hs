...
import Data.Geometry.Structure.Feature (FeatureCollection, GeometryInput (..), SomeJSONInput (..))
...
--      gotScenarioE <- execute (runScenarioGet runLuciService . fst <$> askForScenarioE) >>= switchE
      let gotScenarioF (SRResult _ (LuciResultScenario scId ji t) _) = geoJSONImportFire (Right ji) >> onScenarioGetFire (scId, t)
          gotScenarioF (SRProgress _ _ (Just (LuciResultScenario scId ji t)) _) = geoJSONImportFire (Right ji) >> onScenarioGetFire (scId, t)
...
-- | Luci scenario
data LuciScenario = LuciResultScenario ScenarioId SomeJSONInput UTCTime
instance JS.LikeJS "Object" LuciScenario where
  asLikeJS jsv = case (,) <$> getProp "ScID" jsv <*> getProp "FeatureCollection" jsv of
                  Just (scId, fc) -> LuciResultScenario scId (SJIGeoJSON fc) t
                  Nothing -> anotherTry
     where
  ...
  asJSVal (LuciResultScenario scId ji _) = case ji of 
      ...
      (SJIExtended gi) ->   setProp "ScID"  (JS.asJSVal scId)
