...
data CityGroundView = CityGroundView !Buffer !Buffer !(Maybe Texture)




drawCityGround :: Ctx -> (GLuint,GLuint,GLuint) -> CityGround -> View CityGround -> IO ()
...
updateGroundView gl gr mts (CityGroundView buf ibuf mtex) = do
    ...
    mntex <- case (mts, mtex) of
        ...
        (Just ts, Nothing) -> liftM Just $ initTexture gl ts
